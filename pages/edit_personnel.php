...
Auth::requireAnyPermission([
  'edit_personnel_basic',
  'edit_personnel_course',
  'edit_personnel_education',
  'edit_personnel_service',
  'edit_personnel_family',
  'edit_personnel_health',
  'edit_personnel_social',
  'edit_personnel_notes',
  'edit_personnel_leaves',
  'edit_personnel_cadres',
  'edit_personnel_moqs',
  'edit_personnel_ipft',
  'edit_personnel_yearly_plan',
  'edit_personnel_family_member_status',
  'edit_personnel_status'
]);
$withSidebar = true;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    if (Auth::hasPermission('approval') || Auth::hasPermission('auto_approval') || in_array(Auth::role(), ['admin', 'superadmin', 'techadmin'])) {
      ...
      $data = $_POST;
      if ($photoPath)
        $data['_pending_photo_path'] = $photoPath;
      ...
      <?php if (Auth::hasPermission('edit_personnel_basic')): ?>
        <div class="row mb-4">
          <div class="col-md-3">
            ...
            <img src="<?= htmlspecialchars(personnelPhotoUrl($person['photo_path'] ?? null)) ?>" width="80" height="80"
          ...
          <div class="col-md-3">
            ...
            <input type="text" name="personal_number" class="form-control"
              value="<?= htmlspecialchars($person['personal_number']) ?>" required>
          </div>
          <div class="col-md-2">
            <label class="form-label small">NID</label>
            <input type="text" name="nid" class="form-control" value="<?= htmlspecialchars($person['nid'] ?? '') ?>"
              pattern="^(\d{10}|\d{13}|\d{17})$" title="NID must be 10, 13, or 17 digits">
          </div>
        </div>
          ...
          <div class="col-md-3">
            ...
            <select id="rank_id_edit" name="rank_id" class="form-select">
              ...
              <?php foreach ($ranks as $r): ?>
                <option value="<?= $r['id'] ?>" <?= $person['rank_id'] == $r['id'] ? 'selected' : '' ?>>
                  <?= htmlspecialchars($r['name']) ?></option><?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-3">
            ...
            <select name="unit_id" class="form-select">
              ...
              <?php foreach ($units as $u): ?>
                <option value="<?= $u['id'] ?>" <?= $person['unit_id'] == $u['id'] ? 'selected' : '' ?>>
                  <?= htmlspecialchars($u['name']) ?></option><?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-3">
            ...
            <select name="platoon_id" class="form-select">
              ...
              <?php foreach ($platoons as $p): ?>
                <option value="<?= $p['id'] ?>" <?= $person['platoon_id'] == $p['id'] ? 'selected' : '' ?>>
                  <?= htmlspecialchars($p['name']) ?></option><?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-3">
            ...
            <select id="blood_group_id_edit" name="blood_group_id" class="form-select">
              ...
              <?php foreach ($bloodGroups as $b): ?>
                <option value="<?= $b['id'] ?>" <?= $person['blood_group_id'] == $b['id'] ? 'selected' : '' ?>>
            ...
            <?php if (Auth::hasPermission('edit_personnel_status')): ?>
              <select name="status" class="form-select">
                ...
                foreach ($statusOptions as $val => $label):
                  ?>
                  <option value="<?= $val ?>" <?= $person['status'] === $val ? 'selected' : '' ?>><?= htmlspecialchars($label) ?>
                  </option>
                <?php endforeach; ?>
              </select>
            <?php else: ?>
              <input type="text" class="form-control"
                value="<?= htmlspecialchars(ucwords(str_replace('_', ' ', $person['status'] ?? ''))) ?>" readonly>
            ...
            <div class="col-md-3">
              ...
              <select name="appointment_id" class="form-select">
                ...
                <?php foreach ($appointments as $a): ?>
                  <option value="<?= $a['id'] ?>" <?= ($person['appointment_id'] ?? '') == $a['id'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($a['name']) ?></option><?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-3">
              <label class="form-label small">Mobile number</label>
              <input type="text" name="mobile_number" class="form-control"
                value="<?= htmlspecialchars($person['mobile_number'] ?? '') ?>">
            </div>
            <div class="col-md-3">
              <label class="form-label small">District</label>
              <input type="text" name="address" class="form-control" list="districts_edit"
                value="<?= htmlspecialchars($person['address'] ?? '') ?>" placeholder="Search district">
          ...
          <div class="col-md-8">
            <div class="row g-2">
              <div class="col-md-4">
                ...
                <input type="text" name="vill" class="form-control" value="<?= htmlspecialchars($person['vill'] ?? '') ?>"
                  placeholder="Village">
              </div>
              <div class="col-md-4">
                <label class="form-label small">PO</label>
                <input type="text" name="po" class="form-control" value="<?= htmlspecialchars($person['po'] ?? '') ?>"
                  placeholder="Post Office">
              </div>
              <div class="col-md-4">
                <label class="form-label small">PS</label>
                <input type="text" name="ps" class="form-control" value="<?= htmlspecialchars($person['ps'] ?? '') ?>"
              ...
              <div id="cadre_ids_edit" class="border rounded p-2 bg-white personnel-option-list">
                ...
                <?php foreach ($cadres as $c): ?>
                  ...
                  <div class="form-check py-1">
                    <input class="form-check-input" type="checkbox" name="cadre_ids[]" value="<?= $c['id'] ?>"
                      id="cadre_edit_<?= $c['id'] ?>" <?= $checked ?>>
                  ...
                  <div class="course-item border-bottom py-2 px-1">
                    <div class="d-flex align-items-center justify-content-between">
                      <div class="form-check mb-0">
                        <input class="form-check-input course-checkbox" type="checkbox" id="course_chk_edit_<?= $cid ?>"
                          data-course-id="<?= $cid ?>" <?= $isAdded ? 'checked' : '' ?>>
                        <label class="form-check-label fw-bold small ms-1" for="course_chk_edit_<?= $cid ?>">
                          <?= htmlspecialchars($courseItem['name']) ?>
                        </label>
                      </div>
                      <button type="button"
                        class="btn btn-sm <?= $isAdded ? 'btn-success' : 'btn-outline-primary' ?> py-0 px-2 course-add-btn"
                        data-course-id="<?= $cid ?>">
                        <?= $isAdded ? '✓ Added' : '+ Add' ?>
                      </button>
                    </div>
                    <div class="course-result-container <?= $isAdded ? '' : 'd-none' ?> mt-2 ms-4"
                      id="result_box_edit_<?= $cid ?>">
                      <div class="input-group input-group-sm">
                        ...
                        <input type="text" name="course_result[<?= $cid ?>]" class="form-control course-result-input"
                          placeholder="e.g. Passed, Grade A, Distinction" value="<?= htmlspecialchars($resultVal) ?>"
                          <?= $isAdded ? '' : 'disabled' ?>>
                      </div>
                    </div>
                  </div>
              ...
              <div id="moqs_edit_list" class="border rounded p-2 bg-white personnel-option-list">
                <?php foreach ($moqs as $moqItem): ?>
                  ...
                  <div class="moq-item border-bottom py-2 px-1">
                    <div class="d-flex align-items-center justify-content-between">
                      <div class="form-check mb-0">
                        <input class="form-check-input moq-checkbox" type="checkbox" id="moq_chk_edit_<?= $mid ?>"
                          data-moq-id="<?= $mid ?>" <?= $isAddedMoq ? 'checked' : '' ?>>
                        <label class="form-check-label fw-bold small ms-1" for="moq_chk_edit_<?= $mid ?>">
                          <?= htmlspecialchars($moqItem['name']) ?>
                        </label>
                      </div>
                      <button type="button"
                        class="btn btn-sm <?= $isAddedMoq ? 'btn-success' : 'btn-outline-primary' ?> py-0 px-2 moq-add-btn"
                        data-moq-id="<?= $mid ?>">
                        <?= $isAddedMoq ? '✓ Added' : '+ Add' ?>
                      </button>
                    </div>
                    <div class="moq-result-container <?= $isAddedMoq ? '' : 'd-none' ?> mt-2 ms-4"
                      id="result_box_moq_edit_<?= $mid ?>">
                      <div class="input-group input-group-sm">
                        <span class="input-group-text bg-light text-muted small">Result</span>
                        <input type="hidden" name="moq_id[]" value="<?= $mid ?>" class="moq-id-input" <?= $isAddedMoq ? '' : 'disabled' ?>>
                        <input type="text" name="moq_result[<?= $mid ?>]" class="form-control moq-result-input"
                          placeholder="e.g. Passed, Grade A" value="<?= htmlspecialchars($resultValMoq) ?>" <?= $isAddedMoq ? '' : 'disabled' ?>>
                      </div>
                    </div>
                  </div>
                <?php endforeach; ?>
          ...
          <div class="row mb-3">
            <div class="col-md-3">
              ...
              <input type="date" name="admission_date" class="form-control"
                value="<?= htmlspecialchars($service['admission_date'] ?? '') ?>">
            </div>
            <div class="col-md-3">
              <label class="form-label small">Retirement date</label>
              <input type="date" name="retirement_date" class="form-control"
                value="<?= htmlspecialchars($service['retirement_date'] ?? '') ?>">
            </div>
            <div class="col-md-3">
              <label class="form-label small">UN Mission</label>
              <input type="text" name="un_mission" class="form-control" placeholder="e.g. MINUSCA, MONUSCO"
                value="<?= htmlspecialchars($service['un_mission'] ?? '') ?>">
            </div>
            <div class="col-md-3">
              <label class="form-label small">Punishment note</label>
              <input type="text" name="punishment_note" class="form-control"
                value="<?= htmlspecialchars($service['punishment_note'] ?? '') ?>">
            </div>
          </div>
        <?php endif; ?>
          ...
          <div class="row mb-3">
            <div class="col-md-3">
              ...
              <select name="ipft_1st" class="form-select">
                ...
                <option value="NOT ATTENDING" <?= ($service['ipft_1st'] ?? '') === 'NOT ATTENDING' ? 'selected' : '' ?>>NOT
                  ATTENDING</option>
              </select>
            </div>
            <div class="col-md-3">
              ...
              <select name="ipft_2nd" class="form-select">
                ...
                <option value="NOT ATTENDING" <?= ($service['ipft_2nd'] ?? '') === 'NOT ATTENDING' ? 'selected' : '' ?>>NOT
                  ATTENDING</option>
              </select>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
              ...
              <input type="text" name="speed_march" class="form-control"
                value="<?= htmlspecialchars($service['speed_march'] ?? '') ?>">
            </div>
          </div>
        <?php endif; ?>
            ...
            <?php foreach (['1st', '2nd', '3rd', '4th'] as $idx => $cycle):
              ...
              <div class="col-md-3">
                ...
                <select name="<?= $fieldName ?>" class="form-select">
                  ...
                  <option value="Administration" <?= ($service[$fieldName] ?? '') === 'Administration' ? 'selected' : '' ?>>
                    Administration</option>
                  <option value="Pre Leave" <?= ($service[$fieldName] ?? '') === 'Pre Leave' ? 'selected' : '' ?>>Pre Leave
                  </option>
                  <option value="Group Training" <?= ($service[$fieldName] ?? '') === 'Group Training' ? 'selected' : '' ?>>Group
                    Training</option>
                </select>
              </div>
            <?php endforeach; ?>
            ...
            <div class="row g-2 mb-2 leave-row">
              <div class="col-md-3">
                ...
                <input type="date" name="leaves[from_date][]" class="form-control form-control-sm"
                  value="<?= htmlspecialchars($leave['from_date'] ?? '') ?>">
              </div>
              <div class="col-md-2">
                <label class="form-label small">To</label>
                <input type="date" name="leaves[to_date][]" class="form-control form-control-sm"
                  value="<?= htmlspecialchars($leave['to_date'] ?? '') ?>">
              </div>
              <div class="col-md-2">
                <label class="form-label small">Total Days</label>
                <input type="number" name="leaves[total_days][]" class="form-control form-control-sm"
                  value="<?= htmlspecialchars($leave['total_days'] ?? '') ?>" readonly>
              </div>
              <div class="col-md-3">
                ...
                <select name="leaves[leave_type][]" class="form-select form-select-sm">
                  ...
                  <option value="Weekend Leave" <?= $lt === 'Weekend Leave' ? 'selected' : '' ?>>Weekend</option>
                  <option value="Casual Leave" <?= $lt === 'Casual Leave' ? 'selected' : '' ?>>Casual</option>
                  <option value="Pre Leave" <?= $lt === 'Pre Leave' ? 'selected' : '' ?>>P/L</option>
                  <option value="Medical Leave" <?= $lt === 'Medical Leave' ? 'selected' : '' ?>>Medical</option>
                  <option value="Recreation Leave" <?= $lt === 'Recreation Leave' ? 'selected' : '' ?>>Recreation</option>
                </select>
              </div>
              <div class="col-md-2 d-flex align-items-end">
                <button type="button" class="btn btn-sm btn-outline-danger w-100"
                  onclick="this.closest('.leave-row').remove()"><i class="ti ti-trash me-1"></i> Remove</button>
              </div>
            </div>
          <?php endforeach; ?>
          <?php if (empty($existingLeaves)): ?>
            <div class="row g-2 mb-2 leave-row">
              ...
              <div class="col-md-3">
                ...
                <select name="leaves[leave_type][]" class="form-select form-select-sm">
                  <option value="">-- select --</option>
                  <option value="Weekend Leave">Weekend</option>
                  <option value="Casual Leave">Casual</option>
                  <option value="Pre Leave">Pre</option>
                  <option value="Medical Leave">Medical</option>
                  <option value="Recreation Leave">Recreation</option>
                </select>
              </div>
              <div class="col-md-2 d-flex align-items-end">
                <button type="button" class="btn btn-sm btn-outline-danger w-100"
            ...
            <div class="col-md-3">
              ...
              <select name="marital_status" id="marital_status_select" class="form-select">
                ...
                <?php foreach (['single', 'married', 'widowed', 'divorced'] as $status): ?>
                  <option value="<?= $status ?>" <?= ($family['marital_status'] ?? '') === $status ? 'selected' : '' ?>>
                    <?= ucfirst($status) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-2" id="children_count_container"
              style="display: <?= isset($family['marital_status']) && $family['marital_status'] !== 'single' && $family['marital_status'] !== '' ? 'block' : 'none' ?>;">
              <label class="form-label small">Children count</label>
              <input type="number" min="0" name="children_count" class="form-control"
                value="<?= htmlspecialchars($family['children_count'] ?? '') ?>">
            </div>
          <?php endif; ?>

          <?php if (Auth::hasPermission('edit_personnel_family_member_status')): ?>
            <div class="col-md-2" id="family_member_container"
              style="display: <?= isset($family['marital_status']) && $family['marital_status'] !== 'single' && $family['marital_status'] !== '' ? 'block' : 'none' ?>;">
            ...
            <div class="col-md-2" id="fm_date_from_container"
              style="display: <?= ($family['family_member'] ?? 'No') === 'Yes' ? 'block' : 'none' ?>;">
              <label class="form-label">From Date</label>
              <input type="date" name="fm_date_from" class="form-control"
                value="<?= htmlspecialchars($family['fm_date_from'] ?? '') ?>">
            </div>
            <div class="col-md-2" id="fm_date_to_container"
              style="display: <?= ($family['family_member'] ?? 'No') === 'Yes' ? 'block' : 'none' ?>;">
              <label class="form-label">To Date</label>
              <input type="date" name="fm_date_to" class="form-control"
                value="<?= htmlspecialchars($family['fm_date_to'] ?? '') ?>">
            </div>
            <div class="col-md-12 mt-2" id="fm_current_address_container"
              style="display: <?= ($family['family_member'] ?? 'No') === 'Yes' ? 'block' : 'none' ?>;">
              <label class="form-label small">Currently Living Address</label>
              <input type="text" name="fm_current_address" class="form-control"
                value="<?= htmlspecialchars($family['fm_current_address'] ?? '') ?>">
            </div>
            <div class="col-md-2 mt-2" id="living_status_container"
              style="display: <?= ($family['family_member'] ?? 'No') === 'Yes' ? 'block' : 'none' ?>;">
              <label class="form-label small">Living Status</label>
              <select name="living_status" class="form-select">
                <option value="">-- select --</option>
                <option value="In Living" <?= ($family['living_status'] ?? '') === 'In Living' ? 'selected' : '' ?>>In Living
                </option>
                <option value="Out Living" <?= ($family['living_status'] ?? '') === 'Out Living' ? 'selected' : '' ?>>Out
                  Living</option>
              </select>
            </div>
          <?php endif; ?>
        ...
        <div class="row mb-3">
          <div class="col-md-4">
            ...
            <select name="medical_category_id" class="form-select">
              <option value="">-- select --</option>
              <?php foreach ($medicalCategories as $cat): ?>
                <option value="<?= $cat['id'] ?>" <?= ($health['medical_category_id'] ?? '') == $cat['id'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['name']) ?></option><?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-2">
            <label class="form-label small">Height (cm)</label>
            <input type="number" step="0.1" name="height_cm" class="form-control"
              value="<?= htmlspecialchars($health['height_cm'] ?? '') ?>">
          </div>
          <div class="col-md-2">
            <label class="form-label small">Weight (kg)</label>
            <input type="number" step="0.1" name="weight_kg" class="form-control"
              value="<?= htmlspecialchars($health['weight_kg'] ?? '') ?>">
          </div>
          <div class="col-md-4">
            <label class="form-label small">Any disease</label>
            <input type="text" name="any_disease" class="form-control"
              value="<?= htmlspecialchars($health['any_disease'] ?? '') ?>">
          </div>
        </div>
      <?php endif; ?>
        ...
        <div class="row mb-3">
          <?php foreach (['facebook' => 'Facebook', 'linkedin' => 'LinkedIn', 'whatsapp' => 'WhatsApp', 'twitter' => 'Twitter'] as $key => $label): ?>
            ...
            <div class="col-md-3 mb-3">
              ...
              <input type="<?= $inputType ?>" name="social_links[<?= $key ?>]" class="form-control"
                value="<?= htmlspecialchars($val) ?>" placeholder="<?= $placeholder ?>">
            </div>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
      ...
      <?php if (Auth::hasPermission('edit_personnel_notes')): ?>
        ...
        <div class="mb-3">
          ...
          <textarea name="special_note" class="form-control"
            rows="3"><?= htmlspecialchars($note['note'] ?? '') ?></textarea>
        </div>
      <?php endif; ?>
