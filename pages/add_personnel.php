        ...
        <div class="col-md-4">
          ...
          <select name="appointment_id" class="form-select">
            ...
            <?php foreach ($appointments as $a): ?>
              <option value="<?= $a['id'] ?>" <?= isset($old['appointment_id']) && $old['appointment_id'] == $a['id'] ? 'selected' : '' ?>><?= htmlspecialchars($a['name']) ?></option><?php endforeach; ?>
          </select>
        </div>
      </div>
    </div>
    ...
    <form method="post" enctype="multipart/form-data" class="card shadow-sm p-4">
      ...
      <div class="row mb-4">
        ...
        <div class="col-md-3">
          ...
          <input type="text" name="personal_number" class="form-control"
            value="<?= htmlspecialchars($old['personal_number'] ?? '') ?>" required>
        </div>
        <div class="col-md-3">
          <label class="form-label small">NID</label>
          <input type="text" name="nid" class="form-control" value="<?= htmlspecialchars($old['nid'] ?? '') ?>"
        ...
        <div class="col-md-3">
          ...
          <select id="rank_id_add" name="rank_id" class="form-select">
            ...
            <?php foreach ($ranks as $r): ?>
              <option value="<?= $r['id'] ?>" <?= isset($old['rank_id']) && $old['rank_id'] == $r['id'] ? 'selected' : '' ?>>
        ...
        <div class="col-md-3">
          ...
          <select name="unit_id" class="form-select">
            ...
            <?php foreach ($units as $u): ?>
              <option value="<?= $u['id'] ?>" <?= isset($old['unit_id']) && $old['unit_id'] == $u['id'] ? 'selected' : '' ?>>
        ...
        <div class="col-md-3">
          ...
          <select name="platoon_id" class="form-select">
            ...
            <?php foreach ($platoons as $p): ?>
              <option value="<?= $p['id'] ?>" <?= isset($old['platoon_id']) && $old['platoon_id'] == $p['id'] ? 'selected' : '' ?>><?= htmlspecialchars($p['name']) ?></option><?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-3">
          ...
          <select id="blood_group_id_add" name="blood_group_id" class="form-select">
            ...
            <?php foreach ($bloodGroups as $b): ?>
              <option value="<?= $b['id'] ?>" <?= isset($old['blood_group_id']) && $old['blood_group_id'] == $b['id'] ? 'selected' : '' ?>><?= htmlspecialchars($b['name']) ?></option><?php endforeach; ?>
          </select>
        </div>
      </div>
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="address" class="form-control" list="districts_add"
            value="<?= htmlspecialchars($old['address'] ?? '') ?>" placeholder="Search district">
        ...
        <div class="col-md-8">
          <div class="row g-2">
            <div class="col-md-4">
              ...
              <input type="text" name="vill" class="form-control" value="<?= htmlspecialchars($old['vill'] ?? '') ?>"
                placeholder="Village">
            </div>
            <div class="col-md-4">
              <label class="form-label small">PO</label>
              <input type="text" name="po" class="form-control" value="<?= htmlspecialchars($old['po'] ?? '') ?>"
                placeholder="Post Office">
            </div>
            <div class="col-md-4">
              <label class="form-label small">PS</label>
              <input type="text" name="ps" class="form-control" value="<?= htmlspecialchars($old['ps'] ?? '') ?>"
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-4">
          ...
          foreach ($oldCourses as $idx => $cid) {
            if ($cid)
              $oldCourseMap[$cid] = $oldResults[$idx] ?? ($oldResults[$cid] ?? '');
          }
          ?>
              ...
              <div class="course-item border-bottom py-2 px-1">
                <div class="d-flex align-items-center justify-content-between">
                  <div class="form-check mb-0">
                    <input class="form-check-input course-checkbox" type="checkbox" id="course_chk_add_<?= $cid ?>"
                ...
                <div class="course-result-container <?= $isAdded ? '' : 'd-none' ?> mt-2 ms-4"
                  ...
                  <div class="input-group input-group-sm">
                    ...
                    <input type="text" name="course_result[<?= $cid ?>]" class="form-control course-result-input"
                      placeholder="e.g. Passed, Grade A" value="<?= htmlspecialchars($resultVal) ?>" <?= $isAdded ? '' : 'disabled' ?>>
                  </div>
                </div>
              </div>
            <?php endforeach; ?>
        ...
        <div class="col-md-4">
          ...
          foreach ($oldMoqs as $idx => $mid) {
            if ($mid)
              $oldMoqMap[$mid] = $oldMoqResults[$idx] ?? ($oldMoqResults[$mid] ?? '');
          }
          ?>
          <div id="moqs_add_list" class="border rounded p-2 bg-white personnel-option-list">
            <?php foreach ($moqs as $moqItem): ?>
              ...
              <div class="moq-item border-bottom py-2 px-1">
                <div class="d-flex align-items-center justify-content-between">
                  <div class="form-check mb-0">
                    <input class="form-check-input moq-checkbox" type="checkbox" id="moq_chk_add_<?= $mid ?>"
                      data-moq-id="<?= $mid ?>" <?= $isAdded ? 'checked' : '' ?>>
                    <label class="form-check-label fw-bold small ms-1" for="moq_chk_add_<?= $mid ?>">
                      <?= htmlspecialchars($moqItem['name']) ?>
                    </label>
                  </div>
                  <button type="button"
                    class="btn btn-sm <?= $isAdded ? 'btn-success' : 'btn-outline-primary' ?> py-0 px-2 moq-add-btn"
                    data-moq-id="<?= $mid ?>">
                    <?= $isAdded ? '✓ Added' : '+ Add' ?>
                  </button>
                </div>
                <div class="moq-result-container <?= $isAdded ? '' : 'd-none' ?> mt-2 ms-4"
                  id="result_box_moq_add_<?= $mid ?>">
                  <div class="input-group input-group-sm">
                    <span class="input-group-text bg-light text-muted small">Result</span>
      ...
      <div class="row mb-3">
        <div class="col-md-3">
          ...
          <select name="ipft_1st" class="form-select">
            ...
            <option value="NOT ATTENDING" <?= ($old['ipft_1st'] ?? '') === 'NOT ATTENDING' ? 'selected' : '' ?>>NOT
              ATTENDING</option>
          </select>
        </div>
        <div class="col-md-3">
          ...
          <select name="ipft_2nd" class="form-select">
            ...
            <option value="NOT ATTENDING" <?= ($old['ipft_2nd'] ?? '') === 'NOT ATTENDING' ? 'selected' : '' ?>>NOT
              ATTENDING</option>
          </select>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
          ...
          <input type="text" name="speed_march" class="form-control"
            value="<?= htmlspecialchars($old['speed_march'] ?? '') ?>">
        </div>
      </div>
        ...
        <?php foreach (['1st', '2nd', '3rd', '4th'] as $idx => $cycle):
          ...
          <div class="col-md-3">
            ...
            <select name="<?= $fieldName ?>" class="form-select">
              ...
              <option value="Administration" <?= ($old[$fieldName] ?? '') === 'Administration' ? 'selected' : '' ?>>
                Administration</option>
              <option value="Pre Leave" <?= ($old[$fieldName] ?? '') === 'Pre Leave' ? 'selected' : '' ?>>Pre Leave</option>
              <option value="Group Training" <?= ($old[$fieldName] ?? '') === 'Group Training' ? 'selected' : '' ?>>Group
                Training</option>
            </select>
          </div>
        <?php endforeach; ?>
        ...
        <div class="row g-2 mb-2 leave-row">
          ...
          <div class="col-md-3">
            ...
            <select name="leaves[leave_type][]" class="form-select form-select-sm">
              ...
              <option value="Pre Leave">P/L</option>
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-3">
          ...
          <input type="date" name="marriage_date" class="form-control"
            value="<?= htmlspecialchars($old['marriage_date'] ?? '') ?>">
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-2" id="children_count_container"
          style="display: <?= isset($old['marital_status']) && $old['marital_status'] !== 'single' && $old['marital_status'] !== '' ? 'block' : 'none' ?>;">
          <label class="form-label small">Children count</label>
          <input type="number" min="0" name="children_count" class="form-control"
            value="<?= htmlspecialchars($old['children_count'] ?? '') ?>">
        </div>
        <div class="col-md-2" id="family_member_container"
          ...
          <select name="family_member" id="family_member_select" class="form-select">
            <option value="No" <?= isset($old['family_member']) && $old['family_member'] === 'No' ? 'selected' : '' ?>>No
            </option>
            <option value="Yes" <?= isset($old['family_member']) && $old['family_member'] === 'Yes' ? 'selected' : '' ?>>
              Yes</option>
          </select>
        </div>
        <div class="col-md-2" id="fm_date_from_container"
          style="display: <?= isset($old['family_member']) && $old['family_member'] === 'Yes' ? 'block' : 'none' ?>;">
          <label class="form-label">From Date</label>
          <input type="date" name="fm_date_from" class="form-control"
            value="<?= htmlspecialchars($old['fm_date_from'] ?? '') ?>">
        </div>
        <div class="col-md-2" id="fm_date_to_container"
          style="display: <?= isset($old['family_member']) && $old['family_member'] === 'Yes' ? 'block' : 'none' ?>;">
          <label class="form-label">To Date</label>
          <input type="date" name="fm_date_to" class="form-control"
            value="<?= htmlspecialchars($old['fm_date_to'] ?? '') ?>">
        </div>
        <div class="col-md-12 mt-2" id="fm_current_address_container"
          ...
          <input type="text" name="fm_current_address" class="form-control"
            value="<?= htmlspecialchars($old['fm_current_address'] ?? '') ?>">
        </div>
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="father_mobile" class="form-control"
            value="<?= htmlspecialchars($old['father_mobile'] ?? '') ?>">
        </div>
        <div class="col-md-4">
      ...
      <div class="row mb-3">
        <div class="col-md-4">
          ...
          <select name="medical_category_id" class="form-select">
            ...
            <?php foreach ($medicalCategories as $cat): ?>
              <option value="<?= $cat['id'] ?>" <?= isset($old['medical_category_id']) && $old['medical_category_id'] == $cat['id'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['name']) ?>
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="any_disease" class="form-control"
            value="<?= htmlspecialchars($old['any_disease'] ?? '') ?>">
        </div>
      </div>
      ...
      <div class="mb-3">
        ...
        <textarea name="special_note" class="form-control"
          rows="3"><?= htmlspecialchars($old['special_note'] ?? '') ?></textarea>
      </div>
